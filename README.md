...
**실행 명령어 예시 (따옴표 없이 사용 가능):**
```bash
python python/cli_example.py --id 1234567890 --pw password123 --dep 수서 --arr 부산 --date 20240425 --times 1400,1530 --seat 3 --adult 2 --senior 1
```
...
| `--dep` / `--arr` | 출발역 / 도착역 (기본값: 수서/동대구) | `수서`, `부산` |
| `--date` | 출발 날짜 (YYYYMMDD) | `20240425` |
| `--times` | 타겟팅할 열차 시간 (HHMM, 쉼표 구분) | `1400,1530` |
| `--seat` | 좌석 옵션 (1:일반우선, 2:일반전용, 3:특실우선, 4:특실전용) | `3` |
| `--adult` | 성인 인원수 (기본값: 1, 타 승객 설정 시 0) | `2` |
| `--senior` | 경로 인원수 (기본값: 0) | `1` |

- 공백이 포함되지 않은 파라미터는 따옴표(`"` 또는 `'`) 없이 바로 입력할 수 있습니다.
- `--times`를 입력하지 않으면 조회된 목록에서 수동으로 선택할 수 있는 메뉴가 나타납니다.
- **업데이트 사항**: 다인원 예약 시 승객 유형(어른, 경로 등)이 올바르게 구분되지 않던 문제를 해결했습니다.
- 예약 성공 시 **열차 번호, 좌석 번호, 승객 유형, 결제 기한** 등의 상세 정보가 출력됩니다.
